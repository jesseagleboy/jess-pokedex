			...
			return props.pokemonList.map((Pokemon, index) => {
				return (
					<Link
						...
						<img className={cardsClass[1]} onMouseEnter={cardInfo} src={`${Pokemon.image}`} alt={`${Pokemon.name}`} style={{ borderRadius: "5%" }} />
					</Link>
				);
			});
		}
	...
	return (
		<div className={cardsClass[0]}>
			<div id='deckTexts'>
				...
				<p className='choice' id='water' onMouseOver={deckChoice} onMouseLeave={deckNotChoice}>
					Water-Metal
				</p>
				<p className='choice' id='fire' onMouseOver={deckChoice} onMouseLeave={deckNotChoice}>
					Fighting-Electric
				</p>
				<p className='choice' id='random' onMouseOver={deckChoice} onMouseLeave={deckNotChoice}>
					Random
				</p>
			</div>
